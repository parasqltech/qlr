...
export default () => (
  <StaticQuery
    query={graphql`
      query {
  allWordpressWpApiMenusMenusItems(filter: {slug: {eq: "header-social-link-linked"}}) {
    edges {
      node {
        slug
        items {
      ...
      return (
            ...
            {data &&
              ...
              data.allWordpressWpApiMenusMenusItems.edges[0].node.items.map(
                prop => {
                  return (
                  <ListGroup.Item as="li" className=""><Link className="nav-link p-0" to={prop.url} target="_blank"><i className="fa fa-linkedin"></i></Link></ListGroup.Item>
                  )
                }
              )}
